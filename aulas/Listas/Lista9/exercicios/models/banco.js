...
app.get('/listar-bebidas', async (req, res) => { try { 
...
let bebida = { nome, descricao, fabricante, emEstoque, publicadoEm } 
    try { await Bebidas.create(bebida) 
...
app.get('/listar-lanches', async (req, res) => { try { 
...
app.post('/cadastrar-lanche', async (req, res) => { 
    ...
    let lanche = { nome, descricao, vegano, emEstoque, publicadoEm 
    } 
    try { await Lanches.create(lanche) 
        return res.status(201).json({ 
...
mongoose.connect(connectionString, { dbName: "Casa do lache" }).then(() => { console.log("Conectado ao MongoDB") 
console.log(`Servidor rodando em http://localhost:${port}`) 
app.listen(port) }).catch((error) => { console.log(error) })